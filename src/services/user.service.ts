...
import { getDateFormat, getUserImage } from "../utils"
import fs from "node:fs"
import path from "node:path"
import { PrismaClientKnownRequestError } from "@prisma/client/runtime/library"

export const usersList = async (): Promise<Users[]> => {
  try {
...
export const findUsers = async (userId: string): Promise<Users> => {
  try {
...
export const modifyUser = async (userId: string, body: Users, pic?: Express.Multer.File): Promise<Users> => {
  try {
    ...
    if (pic && !!filename) fs.unlinkSync(path.join('public/images/users', filename.split("/")[3]))
...
export const removeUser = async (userId: string): Promise<Users> => {
  try {
    const filename = await getUserImage(userId)
    ...
    if (!!filename) fs.unlinkSync(path.join('public/images/users', filename.split("/")[3]))
    return result as Users
  } catch (error) {
    if (error instanceof PrismaClientKnownRequestError) {
      if (error.code === 'P2025') {
